...
epoch = 1200  # The number of games we want the system to run for.
...
class ReplayMemory:
    def __init__(self, gridSize, maxMemory, discount):
        ...
        canvas = np.zeros((self.gridSize, self.gridSize))
        canvas = np.reshape(canvas, (-1, self.nbStates))
        self.inputState = np.empty((self.maxMemory, nbStates), dtype=np.float32)
        self.actions = np.zeros(self.maxMemory, dtype=np.uint8)
        self.nextState = np.empty((self.maxMemory, nbStates), dtype=np.float32)
...
def main(_):
    ...
    with tf.Session() as sess:
        ...
        # modelSaveRoot = os.path.join(os.getcwd(), 'model.ckpt')
        if not os.path.isdir(modelSaveRoot):
            os.mkdir(modelSaveRoot)

        save_path = saver.save(sess, os.path.join(modelSaveRoot, "model.ckpt"))
